...
Original file is located at
    https://colab.research.google.com/drive/1pnoCB3unCfy9xAd3nupTeY4Jvy8YvJEZ
"""
...
def evaluate_model(dataX_train, dataY_train, model_type, n_folds=6):
	acc_scores, histories_frozen, histories_unfrozen, histories = list(), list(), list(), list()
	results = []
	best_model = None
	best_accuracy = 0
	...
	for train_ix, test_ix in kfold.split(dataX_train):
		...
		if (model_type == 'CNN_transfer'):
			...
			model.compile(loss='categorical_crossentropy',\
                optimizer='Adam',\
                metrics=['accuracy'])
		elif (model_type == 'CNN_from_scratch'):
			model = define_CNN_model()
		elif (model_type == 'DNN_from_scratch'):
			model = define_DNN_model()
    # Fit model and record history
		if (model_type == 'CNN_transfer' or model_type == 'DNN_transfer'):
			history_frozen = model.fit(train_fold_X, train_fold_Y, epochs=40, batch_size=128, validation_data=(val_fold_X, val_fold_Y), verbose=0)
			histories_frozen.append(history_frozen)
			for layer in model.layers[:-1]:
				layer.trainable = True
			model.compile(loss='categorical_crossentropy',\
									optimizer='Adam',\
									metrics=['accuracy'])
			history_unfrozen = model.fit(train_fold_X, train_fold_Y, epochs=20, batch_size=128, validation_data=(val_fold_X, val_fold_Y), verbose=0)
			histories_unfrozen.append(history_unfrozen)
		else:
			history = model.fit(train_fold_X, train_fold_Y, epochs=150, batch_size=128, validation_data=(val_fold_X, val_fold_Y), verbose=0)
	...
	if (model_type == 'CNN_transfer' or model_type == 'DNN_transfer'):
		return results_df, best_model, acc_scores, histories_unfrozen, histories_frozen
	else:
...
def run_test_harness(model_type):
	if (model_type == 'CNN_transfer' or model_type == 'DNN_transfer'):
		results_df, best_model, scores, histories_unfrozen, histories_frozen = evaluate_model(x_train, y_train, model_type= model_type)
		# learning curves
		print('Frozen all layers, except the classification layer')
		summarize_diagnostics(histories_frozen)
		print('\nFine tuning - Unfreeze all layer')
		summarize_diagnostics(histories_unfrozen)
	else:
...
# Collect results
df1 = run_test_harness(model_type="CNN_from_scratch")
df2 = run_test_harness(model_type="DNN_from_scratch")
df3 = run_test_harness(model_type="CNN_transfer")
df4 = run_test_harness(model_type="DNN_transfer")
results_df = pd.concat([df1, df2, df3, df4], ignore_index=True)
